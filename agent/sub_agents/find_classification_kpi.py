...
from ..prompts import (
    FIND_CLASSIFICATION_KPI_SYSTEM_PROMPT
)
...
system_prompt=Template(FIND_CLASSIFICATION_KPI_SYSTEM_PROMPT).substitute(
    clarifications_allowed=os.getenv('FIND_CLASSIFICATION_KPI_CLARIFICATIONS')
)

# Initialize the agent with flexible model configuration
find_classification_kpi_agent = Agent(
    get_llm_model_for_agent(model_choice=os.getenv('FIND_CLASSIFICATION_KPI_AGENT_LLM_CHOICE')),
    deps_type=AgentDependencies,
    system_prompt=system_prompt
)


@find_classification_kpi_agent.tool
